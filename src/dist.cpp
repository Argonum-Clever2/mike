...
Distance::Distance(std::string dirpathNew){
    dirpath=dirpathNew;

}

bool Distance::getFileType(std::string file){
    std::string fileType;
...
float Distance::getFileCFile(std::string file_1, std::string file_2, int paiLen){
    std::ifstream is1(file_1, std::ios::in);
    std::ifstream is2(file_2, std::ios::in);

    if (!is1.good()){
        std::cerr<<"[ERROR]:    "<<file_1<<" is error, please input the right file! "<<std::endl;
        exit(-1);
    }
    if (!is2.good()){
        std::cerr<<"[ERROR]:    "<<file_2<<" is error, please input the right file! "<<std::endl;
        exit(-1);
    }
    ...
    for(int i=0; i < paiLen; i++){
        ...
        dist = (float)1- pow((2*jaccard)/(1+jaccard), (float)1/21);
    }
...
bool getF2L(std::string file, std::string filelist, int paiLen, Distance &distance){
    ...
    if(distance.getFileType(file)){
        std::cerr<<"the input file is error! ! !"<<std::endl;
        return false;
    }

    // 输出文件
    std::string output=distance.dirpath+"/dist.txt"; 
    ...
    while (!list.eof()){
        getline(list, suftmp);
        if (suftmp == "") break;
        if (distance.getFileType(suftmp)){
    ...
    ot<< name;
    for(int i=0; i < filelistArr.size(); i++){
        dist=distance.getFileCFile(file, filelistArr[i], paiLen);
        ot<<'\t'<<dist;
    }
    ot<<std::endl;
    ot.close();
...
bool getL2L(std::string filelist_1, std::string filelist_2, int paiLen, Distance &distance){
    ...
    for(int i=0; i < filelistArr_1.size(); i++){
        ...
        for(int j=0; j < filelistArr_2.size(); j++){
            dist=distance.getFileCFile(filelistArr_1[i], filelistArr_2[j], paiLen);
            if (j == (filelistArr_2.size()-1)){
                ot<<dist;
            }else{
                ot<<dist<<'\t';
            }
        ...
        curr_tm = localtime(&now);
        strftime(timeArr, 80, "%Y-%m-%d %H:%M:%S", curr_tm);
        std::cout<<timeArr<<"    No."<<i<<'\t'<<name<<" has finished. "<<std::endl;
    }
...
bool getF2F(std::string file_1, std::string file_2, int paiLen, Distance &distance){
    ...
    float dist;
    if ((distance.getFileType(file_1))&&(distance.getFileType(file_2))){
        dist = distance.getFileCFile(file_1, file_2, paiLen);
        std::cout<<"The Mash evolution distance between "<<fileName_1<<" and "<<fileName_2<<" is "<<dist<<std::endl;
        return true;
    }else{
...
//     int paiLen=1;
//     //getF2F(file_1, file_2, paiLen, distance);
//     // getF2L(file_1, list_2, paiLen, distance);
//     getL2L(list_1, list_2, paiLen, distance);
